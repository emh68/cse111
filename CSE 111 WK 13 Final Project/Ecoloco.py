...
def generate_tone(frequency, volume, duration, sample_rate=44100):
    ...
    left_samples = (volume * np.sin(2 * np.pi * frequency * t)).astype(np.float32)
    right_samples = (volume * np.sin(2 * np.pi * frequency * t + np.pi)).astype(np.float32)
...
class ToneGenerator:
    ...
    def callback(self, in_data, frame_count, time_info, status):
        if self.samples:
            data = self.samples[:frame_count * self.channels * self.sample_width]
            self.samples = self.samples[frame_count * self.channels * self.sample_width:]
...
def get_distance_to_target(dtt):
    ...
    if 144 <= dtt <= 180:
        ...
        generator.play_samples(samples, channels=2, sample_width=generator.p.get_sample_size(pyaudio.paFloat32))
    else:
        ...
        samples = generate_tone(440, volume, tone_duration)
        generator.play_samples_with_repetitions(samples, channels=2, sample_width=generator.p.get_sample_size(pyaudio.paFloat32), repetitions=repetitions, time_between_tones=time_between_tones, reduced_sleep=(12 <= dtt <= 23))
