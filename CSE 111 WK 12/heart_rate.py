...
def main():
    ...
    frm_main.pack(padx=144, pady=133, fill=tk.BOTH, expand=1)
...
def populate_main_window(frm_main):
    ...
    # ent_length = IntEntry(frm_main, width=4, lower_bound=12, upper_bound=90)

    # Create a label that displays "years"
    lbl_age_units = Label(frm_main, text="Enter Age:")

    # Create a label that displays "Rates:"
    lbl_rates = Label(frm_main, text="Heart Rates:")

    # Create labels that will display the results.
    lbl_slow = Label(frm_main, width=3)
    lbl_fast = Label(frm_main, width=3)
    lbl_rate_units = Label(frm_main, text="beats/min")
    ...
    ent_age.grid(row=0, column=1, padx=0, pady=0)
    lbl_age_units.grid(row=0, column=0, padx=0, pady=0)

    lbl_rates.grid(row=1, column=0, padx=0, pady=20)
    ...
    btn_clear.grid(row=2, column=2, padx=0, pady=20, columnspan=4, sticky="w")
