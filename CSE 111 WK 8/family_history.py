...
def print_death_age(people_dict):
    ...
    for person_key, person_list in people_dict.items():
        ...
        # Calculate person's age at death.
        death_age = death_year - birth_year
        print(name, death_age, birth_year, death_year)
...
def count_genders(people_dict):
    ...
    females = 0
    for person_key, person_list in people_dict.items():
        if person_list[GENDER_INDEX] == "M":
            males += 1
        elif person_list[GENDER_INDEX] == "F":
            females += 1
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage_key, marriage_list in marriages_dict.items():
        ...
        husband_death = husband_list[DEATH_YEAR_INDEX]

        husband_wedding_age = wedding_year - husband_birth
        husband_age_at_death = husband_death - husband_birth

        wife_list = people_dict[wife_key]
        wife_name = wife_list[NAME_INDEX]
        wife_birth = wife_list[BIRTH_YEAR_INDEX]
        wife_death = wife_list[DEATH_YEAR_INDEX]

        wife_wedding_age = wedding_year - wife_birth
        wife_age_at_death = wife_death - wife_birth

        # print(f"{husband_name}, {husband_age}, {wedding_year}, {wife_name}, {wife_age}")
        husband_wife_table = PrettyTable(["Name", "Wedding Age", "Wedding Year", "Death Age"])
        husband_wife_table.add_row([husband_name, husband_wedding_age, wedding_year, husband_age_at_death])
        husband_wife_table.add_row([wife_name, wife_wedding_age, wedding_year, wife_age_at_death])
        print(husband_wife_table)
